...
export function EditSale() {
  ...
  useEffect(() => {
    async function handleGetSale() {
      ...
      if (saleData) {
        setCpfCnpj(saleData?.cpfCnpj || "");
        setRegion(saleData?.region || "");
        setTicket(saleData?.ticket || "");
        setCallerIdPhone(saleData?.callerIdPhone || "");
        setPhone(saleData?.phone || "");
        setInternetPlanSpeed(saleData?.internetPlanSpeed || "");
        setPaymentMethod(saleData?.paymentMethod || "");
        setInternetType(saleData?.internetType || "");
        setInstallationShift(saleData?.installationShift || "");
        setCustomerName(saleData?.customerName || "");
        setServiceOrder(saleData?.serviceOrder || "");
        setExtension(saleData?.extension || "");
        setStatus(saleData?.status || "Em aprovisionamento");

        if (saleData?.saleDate) {
          const formattedSaleDate = new Date(saleData.saleDate)
            .toISOString()
            .split("T")[0];
        ...
        if (saleData?.installationDate) {
          const formattedInstallationDate = new Date(saleData.installationDate)
            .toISOString()
            .split("T")[0];
  ...
  async function handleEditSale(e: any) {
    e.preventDefault();
        ...
        <div>
          ...
          <div>
            <div className="buttons">
              <NewSaleBtn type="submit">Editar Venda</NewSaleBtn>
